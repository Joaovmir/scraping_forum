...
def mostra_top(qtd):
    ...
    ax = sns.barplot(x='Tópicos sem resposta',y= 'Categoria', data = dados.head(qtd), palette = lista_cores[0:qtd])
...
def mostra_areas_estudo():
    ...
    sns.set_style('darkgrid')
    sns.set_context("notebook", font_scale=1.2)
    ax = sns.barplot(x='Tópicos sem resposta', y= tabela_areas_estudo.index,  data = tabela_areas_estudo, palette=lista_cores)
    ax.figure.set_size_inches(14,6)
